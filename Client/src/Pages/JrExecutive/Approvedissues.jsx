...
const ApprovedIssues = () => {
  ...
  useEffect(() => {
    const fetchApprovedIssues = async () => {
      try {
        ...
        setIssues(data.filter(i => i.jrExecutiveStatus.status === 'Approved'));
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(i =>
        i.vehicle?.name?.toLowerCase().includes(term) ||
        i.description.toLowerCase().includes(term)
      );
    }

    if (filterOICStatus !== 'all') {
      filtered = filtered.filter(i => i.oicStatus.status === filterOICStatus);
    }
  ...
  const countByStatus = status =>
    issues.filter(i => i.oicStatus.status === status).length;
                  ...
                  {filteredIssues.map(issue => (
                    <tr key={issue._id} className="table-row">
                      ...
                      <td className="py-4 px-4 text-military-400 text-sm truncate max-w-xs">
                        {issue.jrExecutiveStatus.comment || 'No comment'}
                      </td>
                      <td className="py-4 px-4">
                        {getStatusBadge(issue.oicStatus.status)}
                      </td>
