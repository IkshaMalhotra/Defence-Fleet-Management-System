...
const SupplierDashboard = () => {
  ...
  const openUpdateModal = (request) => {
    ...
    setSupplyStatus(request.supplierStatus?.status || 'Pending');
    setComment(request.supplierStatus?.comment || '');
    setShowModal(true);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="card p-6 border-l-4 border-amber-500">
            ...
            <p className="text-3xl font-bold text-military-100">
              {requests.filter(r => !r.supplierStatus || r.supplierStatus.status === 'Pending').length}
            </p>
          </div>
          <div className="card p-6 border-l-4 border-green-500">
            <h3 className="text-military-400 text-sm mb-2">Supplied</h3>
            <p className="text-3xl font-bold text-military-100">
              {requests.filter(r => r.supplierStatus?.status === 'Supplied').length}
            </p>
          </div>
          <div className="card p-6 border-l-4 border-red-500">
            <h3 className="text-military-400 text-sm mb-2">Not Available</h3>
            <p className="text-3xl font-bold text-military-100">
              {requests.filter(r => r.supplierStatus?.status === 'Not Available').length}
            </p>
          </div>
        </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          {getSupplyStatusBadge(request.supplierStatus?.status)}
                        </div>
                      </div>

                      {/* Approval Comments */}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        {request.jrExecutiveStatus.comment && (
                          <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                            <p className="text-xs text-green-400 mb-1">Jr Executive:</p>
                            <p className="text-military-300 text-sm">{request.jrExecutiveStatus.comment}</p>
                          </div>
                        )}
                        {request.oicStatus.comment && (
                          <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                            <p className="text-xs text-green-400 mb-1">OIC:</p>
                            <p className="text-military-300 text-sm">{request.oicStatus.comment}</p>
                          </div>
                        )}
                      </div>

                      {/* Supplier Comment if exists */}
                      {request.supplierStatus?.comment && (
                        <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                          <p className="text-xs text-blue-400 mb-1">Your Comment:</p>
                          <p className="text-military-300 text-sm">{request.supplierStatus.comment}</p>
                        </div>
                      )}
